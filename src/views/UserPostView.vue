...
export default {
    ...
    methods: {
        // get users posts
        getUserPosts() {
            axios.get(config.base_url+'posts/showbyuser', { headers: {"Authorization" : `Bearer ${localStorage.getItem('user-token')}`} }).then((response)=>{
        ...
        // delete post
        deletePost(id, index) {
            axios.delete(config.base_url+'posts/'+id, { headers: {"Authorization" : `Bearer ${localStorage.getItem('user-token')}`} }).then((response)=>{
                        ...
                        <div class="row">
                            ...
                            <div class="col-4">
                                <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                    ...
                                    <button type="button" class="btn btn-danger" @click="deletePost(post.id, index)">Delete</button>
                                </div>
                            </div>
                        </div>
                    </li>
